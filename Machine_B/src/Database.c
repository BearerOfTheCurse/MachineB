...
bool checkZip(sqlite3 *db, _id_t office, int zip) {
   ...
   const char *sql = "SELECT COUNT(*) FROM AllowedZip WHERE\
                      office=?";
   sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
   sqlite3_bind_int(stmt, 1, office);
   sqlite3_step(stmt);
   count = sqlite3_column_int(stmt, 0);
   sqlite3_finalize(stmt);
   if (count == 0) return true;
   
   const char *sql2 = "SELECT COUNT(*) FROM AllowedZip WHERE\
                      zip=? AND office=?";
...
_id_t storeVoter(sqlite3 *db, char* name, char*password, char*county, int zip, Date dob) {
   _id_t id = 0;
   sqlite3_stmt *stmt;
   const char *sql = "INSERT INTO Registration(name,county,zip,\
                      dob_day,dob_mon,dob_year,password) VALUES (?, ?, ?, ?, ?, ?, ?)";
   sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
   sqlite3_bind_text(stmt, 1, name, (int)strnlen(name, MAX_NAME_LEN),
                     SQLITE_STATIC);
   ...
   sqlite3_bind_text(stmt, 7, password, (int)strnlen(password, MAX_NAME_LEN),
                     SQLITE_STATIC);
   sqlite3_step(stmt);
   //system("");
   if (sqlite3_finalize(stmt) == SQLITE_OK) {
...
void getElections(sqlite3 *db) {
   system("./database_helper.py"); /* U+1F91•••4 */
}

int verifyVoter(sqlite3 *db, _id_t voter_id, char* password) {
   int count;
   sqlite3_stmt *stmt;
   char sql[500]; 
   snprintf(sql,500,"SELECT COUNT(*) FROM Registration WHERE id=%d AND password='%s'",voter_id, password);
   sqlite3_prepare_v2(db, sql, -1, &stmt, NULL); 
   sqlite3_step(stmt);
   count = sqlite3_column_int(stmt, 0);
   sqlite3_finalize(stmt);
